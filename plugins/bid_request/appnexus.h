...
namespace AppNexus {
  ...
  struct DeviceIds {
    string idfa;     // "ID for ad" for iOS devices, used by iOS 6 and up
    string sha1udid; // SHA1 hash of ANDROID_ID
    string md5udid;  // MD5 hash of ANDROID_ID
    string sha1mac;  // SHA1 hash of ODIN. Used by iOS 5 and below
    string openudid; // OpenUDID of the device. Used by iOS 5 and below
  ...
  struct Tag {
    ...
    Id siteId;
    Id inventorySourceId;  // value may not be in request sent to bidder if bidder configur to hide
    // /Subsection: Inventory hierarchy data
    // Subsection: General data
    string size;            // ex. 300x160 for impression, soon deprecated in favor of "sizes" field
    vector<string> sizes;   // first size is the "primary" size, other sizes must be smaller
    AdPosition position;    // enum "below", "above", "unknown"
  ...
  struct BidInfo {
    ...
    Id browser;  // ID of user's browser
    ...
    TaggedInt carrier;    // ID of carrier
    TaggedInt deviceMake; // ID of the device make
    TaggedInt deviceModel;// ID of the device model
    TaggedInt deviceType; // Enum: 1 = "pc", 2 = "phone", 3 = "tablet"
    DeviceIds deviceIds;  // IDs associates with the impression up for bid
    // /Subsection: Mobile fields not available in initial release
  ...
  struct BidRequestRoot {
    // Artificial Tag resolver
